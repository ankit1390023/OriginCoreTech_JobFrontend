...
const FeedTerms = () => {
  ...
  useEffect(() => {
    const fetchTermsAndConditions = async () => {
      try {
        ...
        setTermsData(response.termsAndCondition);
        setEditData(response.termsAndCondition);
        setError(null);
      } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      let errorMessage = 'Failed to update terms and conditions. Please try again.';
  ...
  return (
    ...
    <section className="flex justify-center items-start pt-2 ">
      <div 
        className="bg-white rounded-[10px] p-5 shadow-lg mt-1 w-[800px] h-[1000px] opacity-100 gap-[10px]"
      >
