...
const FeedYourEducation = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            const specializationDropdowns = document.querySelectorAll('[data-specialization-dropdown]');
    ...
    const handleCollegeSelect = (id, college) => {
        handleInputChange(id, 'collegeName', college);
                                    ...
                                    {showCourseDropdown && searchQuery.trim() && (
                                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                            ...
                                            {educationData.courses.filter(course =>
                                                ...
                                                <div className="px-3 py-2 text-gray-400 text-sm">No courses found</div>
                                            )}
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="space-y-3 sm:space-y-4 px-4 sm:px-6 pb-4 sm:pb-6">
                                {educationEntries.map((entry, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 gap-2 sm:gap-3 pt-3 sm:pt-4 px-3 sm:px-4">
                                            ...
                                            <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-600">
                                                {entry.certificate ? (
                                                    ...
                                                    >
                                                        <span className="hidden sm:inline">View/Edit Certificate</span>
                                        ...
                                        {/* Upload Progress Bar */}
                                        {uploadProgress[entry.id] !== undefined && uploadProgress[entry.id] < 100 && (
                                            <div className="px-3 sm:px-4 mb-2">
                                                ...
                                                <p className="text-xs text-gray-600 mt-1">Uploading... {uploadProgress[entry.id]}%</p>
                                            </div>
                                        )}
                                        
                                        {/* Form Fields */}
                                        <div className="w-full rounded-lg p-2 sm:p-3 opacity-100 flex flex-col justify-between" style={{ gap: '4px' }}>
                                            <div className="flex flex-col gap-2 sm:gap-3">
                                                ...
                                                <div className="relative" data-college-dropdown>
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => handleInputChange(entry.id, 'collegeName', e.target.value)}
                                                        onFocus={() => setShowCollegeDropdown(prev => ({ ...prev, [entry.id]: true }))}
                                                    ...
                                                    {showCollegeDropdown[entry.id] && (
                                                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                            <input
                                                                ...
                                                                onChange={(e) => setCollegeSearchQuery(prev => ({ ...prev, [entry.id]: e.target.value }))}
                                                                className="w-full px-3 py-2 border-b border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                                autoFocus
                                                            />
                                                            {filteredColleges('', entry.id).map((college, index) => (
                                                                <div
                                                                    key={index}
                                                                    className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm"
                                                                    onClick={() => handleCollegeSelect(entry.id, college)}
                                                                >
                                                                    {college}
                                                                </div>
                                                            ))}
                                                ...
                                                <div className="relative" data-specialization-dropdown>
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => handleInputChange(entry.id, 'specialization', e.target.value)}
                                                        onFocus={() => setShowSpecializationDropdown(prev => ({ ...prev, [entry.id]: true }))}
                                                        className="bg-transparent w-full h-10 sm:h-12 rounded-lg px-3 sm:px-4 opacity-100 border border-gray-200 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                    />
                                                    {showSpecializationDropdown[entry.id] && (
                                                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                            <input
                                                                ...
                                                                onChange={(e) => setSpecializationSearchQuery(prev => ({ ...prev, [entry.id]: e.target.value }))}
                                                                className="w-full px-3 py-2 border-b border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                                autoFocus
                                                            />
                                                            {filteredSpecializations('', entry.id).map((specialization, index) => (
                                                                <div
                                                                    key={index}
                                                                    className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm"
                                                                    onClick={() => handleSpecializationSelect(entry.id, specialization)}
                                                                >
                                                                    {specialization}
                                                                </div>
                                                            ))}
                        ...
                        <div>
                          ...
                          <div className="">
                            <Select
                              ...
                              onChange={(e) => handleInputChange(entry.id, 'startYear', e.target.value)}
                        ...
                        <div>
                          ...
                          <div className="">
                            <Select
                              ...
                              options={yearOptions}
                              value={entry.endYear}
                              onChange={(e) => handleInputChange(entry.id, 'endYear', e.target.value)}
                              className="w-full h-12 rounded-lg px-4 border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                            />
                            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
                              <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div className="p-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-900">{selectedCertificate.certificateName}</p>
                                    </div>
                                    ...
                                    {selectedCertificate.certificateFile && (
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-900">
                                                Size: {formatFileSize(selectedCertificate.certificateFile.size)}
                                            </p>
                                            <p className="text-sm text-gray-900">
                                    ...
                                    <div className="flex gap-3 pt-4">
                                        <button
                                            onClick={() => {
                                                ...
                                                link.href = selectedCertificate.certificateFile ? URL.createObjectURL(selectedCertificate.certificateFile) : '#';
                                    ...
                                    <div className="pt-4">
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Replace Certificate</label>
                                        <input
                                            type="file"
                                            accept=".pdf,.jpg,.jpeg,.png"
                                            onChange={(e) => {
