...
export const skillApi = {
    ...
    uploadSkills: async (user_id, skills, certificateFiles, token) => {
        try {
            ...
            if (certificateFiles && Array.isArray(certificateFiles) && certificateFiles.length > 0) {
                certificateFiles.forEach((file) => {
                    if (file && typeof file === 'string') {
                        ...
                        console.warn('File object detected, should be uploaded separately first');
                    }
                });
            }
            ...
            console.log("error.response.status from uploadSkills", error.response.status);
            throw error;
        }
    },
