...
export const useApplyToJob = () => {
    ...
    const applyToJob = async (jobId, applicationData) => {
        ...
        try {
            ...
            console.log('Applying for job with data:', { jobId, data });
            console.log('Token available:', !!token);
            console.log('Token value:', token ? token.substring(0, 10) + '...' : 'No token');
            
            const response = await applicationApi.applyForJob(jobId, data, token);
            console.log('Application response:', response);
            
            if (response.success) {
                setSuccess(true);
                return { success: true, message: response.message || 'Application submitted successfully!' };
            } else {
