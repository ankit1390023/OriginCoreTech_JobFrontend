...
export const useSkillApi = () => {
  ...
  const uploadSkills = useCallback(
    async (user_id, skills, certificateFiles = []) => {
      if (!token) {
      ...
      const duplicatedCertificates = Array(skills.length).fill(
        certificateFiles[0]
      );
      ...
      try {
        ...
        const response = await skillApi.uploadSkills(
          user_id,
          skills,
  ...
  const getUserSkills = useCallback(
    async (user_id) => {
      ...
      try {
        setLoading(true);
        setError(null);
        const response = await skillApi.getUserSkills(user_id, token);
        return response;
      } catch (err) {
